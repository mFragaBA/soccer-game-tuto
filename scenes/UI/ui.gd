...
func update_flags() -> void:
	flag_texture[0].texture = FlagHelper.get_texture(GameManager.current_match.team_home)
	flag_texture[1].texture = FlagHelper.get_texture(GameManager.current_match.team_away)
...
func on_game_over(_winning_team: String) -> void:
