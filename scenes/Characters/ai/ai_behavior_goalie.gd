...
func perform_ai_decisions() -> void:
	...
	if ball.is_headed_for_scoring_area(player.own_goal.scoring_area):
		player.switch_state(Player.State.DIVING)
	
func get_goalie_steering_force() -> Vector2:
	var top := player.own_goal.get_top_position()
	var bottom := player.own_goal.get_bottom_position()
	var center := player.spawn_position
	
	# Y axis becomes negative upwards so top.y is the smaller value
